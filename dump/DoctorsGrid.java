...
public class DoctorsGrid extends GridForm<Doctor> {
  ...
  public DoctorsGrid(DoctorRepository doctorRepository) {
    ...
    EditDoctorDialog editDoctorDialog = new EditDoctorDialog("Врач", UI.getCurrent(), doctor, doctorRepository);
  
    if(doctorId > 0){
      doctorRepository.findById(doctorId).ifPresent(e->doctor = e);
    }
    
    addButton.addClickListener(e -> editDoctorDialog.open());
    editButton.addClickListener(e -> {
      if(doctorId > 0){
        editDoctorDialog.open();
      }
    });
